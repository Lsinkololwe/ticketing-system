...
const BLACKLIST_PREFIX = 'pml-admin:blacklist:';      // Token JTI blacklist
const USER_SESSIONS_PREFIX = 'pml-admin:user:';       // User → Sessions index
const SESSION_PREFIX = 'pml-admin:session:';          // Session storage
const JTI_SESSION_PREFIX = 'pml-admin:jti:';          // JTI → Session mapping

// Default TTL for blacklisted tokens (15 minutes = typical access token lifetime)
 ...
 *
 * The token will be blacklisted until its original expiration time.
 * After that, the blacklist entry is automatically removed by Redis TTL.
 *
...
export async function blacklistToken(jti: string, expiresAt: number): Promise<boolean> {
  ...
  try {
    ...
    if (ttl <= 0) {
      ...
      console.log(`[TokenBlacklist] Token ${maskJti(jti)} already expired, skipping`);
      return false;
    }

    // SET with EX (expire) - O(1) operation
    await client.set(`${BLACKLIST_PREFIX}${jti}`, '1', 'EX', ttl);

    console.log(`[TokenBlacklist] Blacklisted token ${maskJti(jti)} for ${ttl}s`);
    return true;
  } catch (error) {
 ...
 *
 * Extracts jti and exp claims from the token and blacklists it.
 *
 * @param token - Raw JWT string
 * @returns true if blacklisted successfully
 */
export async function blacklistTokenFromJwt(token: string): Promise<boolean> {
  try {
    ...
    if (!exp) {
      // Use default TTL if no exp claim
      const defaultExp = Math.floor(Date.now() / 1000) + DEFAULT_BLACKLIST_TTL_SECONDS;
      return blacklistToken(jti, defaultExp);
    }

    return blacklistToken(jti, exp);
  } catch (error) {
 ...
 */
const USER_REVOCATION_PREFIX = 'pml-admin:revoked:';
